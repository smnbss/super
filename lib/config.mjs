...
function globalConfigPath() {
  const path = join(process.env.HOME, '.super', CONFIG_FILE);
...
function mergeCatalogSection(baseSection = {}, projectSection = {}) {
  const merged = { ...baseSection };
  for (const [name, value] of Object.entries(projectSection || {})) {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
      merged[name] = value;
      continue;
    }
    if (!merged[name] || typeof merged[name] !== 'object' || Array.isArray(merged[name])) {
      merged[name] = value;
      continue;
    }
    merged[name] = { ...merged[name] };
    if (Object.prototype.hasOwnProperty.call(value, 'enabled')) {
      merged[name].enabled = value.enabled;
    }
  }
  return merged;
}
...
function loadConfig(from) {
  ...
  const projectConfig = loadYamlFile(path);
  const globalPath = globalConfigPath();

  if (!globalPath || globalPath === path) {
    _configCache = projectConfig;
    _configPath = path;
    return _configCache;
  }

  const globalConfig = loadYamlFile(globalPath);
  const merged = mergeObjects(globalConfig, projectConfig);
  for (const section of ['system', 'clis', 'skills', 'plugins', 'commands', 'mcps', 'remote_mcps']) {
    merged[section] = mergeCatalogSection(globalConfig[section], projectConfig[section]);
  }
