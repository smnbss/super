...
function updateSessionHeader(file, userContent) {
  ...
  // Extract current CLI and model for terminal title update
  const cliMatch = content.match(/^\*\*CLI:\*\* (.+)$/m);
  const modelMatch = content.match(/^\*\*Model:\*\* (.+)$/m);
  const cli = cliMatch?.[1] || 'unknown';
  ...
  // Update terminal title when session title changes
  updateTerminalTitle(cli, title, model);
}
...
export function updateTerminalTitle(cli, title, model) {
  const icon = { claude: '🟠', gemini: '🔵', codex: '🟢', antigravity: '🪽' }[cli] || '⚪';
  const modelSuffix = model ? ` (${model})` : '';
  const tabTitle = `${icon} ${title}${modelSuffix}`;

  // OSC 0: Set window title and icon (widely supported)
  // OSC 2: Set window title (alternative)
  try {
    process.stdout.write(`\x1b]0;${tabTitle}\x07`);
    process.stdout.write(`\x1b]2;${tabTitle}\x07`);
...
export function sessionNew(title = 'untitled', cli = 'unknown', provider = '', model = '', from) {
  const safeTitle = title.toLowerCase()
    .replace(/[^a-z0-9_-]/g, '-').replace(/-+/g, '-')
    .replace(/^-|-$/g, '').slice(0, 40) || 'untitled';

  const now = new Date();
  const ts = now.toISOString().slice(0, 23).replace(/T/, '_').replace(/[:.]/g, '');
  const dir = sessionsDir(from);
  mkdirSync(dir, { recursive: true });

  let filename = `${ts}.md`;
  let filepath = join(dir, filename);
  // Collision-safe: bump suffix if a session with the same millisecond already exists
  let suffix = 1;
  while (existsSync(filepath)) {
    filename = `${ts}-${suffix}.md`;
    filepath = join(dir, filename);
    suffix++;
  }
  const root = findRoot(from);

  // Build metadata lines conditionally
  const cliLine = `**CLI:** ${cli}`;
...
**File:** ${filename}
${cliLine}${providerLine}${modelLine}
...
export function sessionList(from) {
  ...
  return readdirSync(dir)
    ...
    .map(f => {
      ...
      const cliMatch = content.match(/^\*\*CLI:\*\* (.+)$/m);
      const providerMatch = content.match(/^\*\*Provider:\*\* (.+)$/m);
      const modelMatch = content.match(/^\*\*Model:\*\* (.+)$/m);
...
function contextFile(from) {
  return join(baseDir(from), 'session-context.md');
}
...
export function sessionClearInjections(from) {
  ...
  if (existsSync(target)) {
    unlinkSync(target);
    log(`Cleared context → ${target}`, from);
  }
}
