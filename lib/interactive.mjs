...
export async function selectSingle(title, options, defaultIdx = 0) {
  ...
  if (process.stdin.isTTY) return arrowsSingle(title, options, defaultIdx);
...
function gumSingle(title, options) {
  try {
    const result = execSync(
      `gum choose --height=10 --header ${JSON.stringify(title)} -- ${options.map(o => JSON.stringify(o)).join(' ')}`,
...
export async function selectMulti(title, options) {
  ...
  if (process.stdin.isTTY) return arrowsMulti(title, options);
...
function gumMulti(title, options) {
  try {
    const result = execSync(
      `gum choose --no-limit --height=10 --header ${JSON.stringify(title)} -- ${options.map(o => JSON.stringify(o)).join(' ')}`,
      { encoding: 'utf8', stdio: ['inherit', 'pipe', 'pipe'] }
    ).trim();
...
export async function input(prompt, defaultValue = '') {
  if (hasGum) {
    try {
      const result = execSync(
        `gum input --header ${JSON.stringify(prompt)} --value ${JSON.stringify(defaultValue)}`,
        { encoding: 'utf8', stdio: ['inherit', 'pipe', 'pipe'] }
      ).trim();
