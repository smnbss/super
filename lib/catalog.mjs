...
// Creates a debug-only convenience link: <project>/.CLI/.CLI → ~/.CLI.
// It lets developers cd into the CLI's global home from inside the project.
// NOTHING in super or its skills should ever read, write, enumerate, or descend
// through these `.CLI/.CLI` paths — they are not part of the project, and
// following them corrupts walk-up discovery and config writes.
...
export function scaffoldEnvLocal() {
  ...
  if (!existsSync(envLocal)) {
    if (template && existsSync(template)) {
      ...
      ui.success('  .env.local created — edit it to add your secrets before running super configure');
    } else {
...
export function installPhaseInstall(selectedClis) {
...
// Configure phase. Runs everything `super configure` (called by /super-setup)
// needs after the user has filled in `.env.local`: external catalog skills,
// plugins, skill-dir sync (idempotent re-run so newly-installed skills appear
// in CLI dirs), and MCPs.
