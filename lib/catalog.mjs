...
function ensureClaudeSkillsSymlink() {
  ...
  if (existsSync(claudeSkills)) {
    ...
    if (stat && !stat.isSymbolicLink()) {
      ui.muted(`  .claude/skills exists as a directory — leaving it alone`);
      return;
    }
  ...
  try {
    ...
    ui.success(`  .claude/skills → .agents/skills`);
  } catch (err) {
    ui.warn(`  Failed to create .claude/skills symlink: ${err.message}`);
  }
}
...
function installPluginSkills(name, source) {
  const root = findRoot();
  const agentsSkills = join(root, '.agents', 'skills');
  if (!existsSync(agentsSkills)) return;
  ...
  try {
    if (!cloneSkillSource(name, source, dest)) return;

    const skillsDir = join(dest, 'skills');
    if (existsSync(skillsDir)) {
      // Plugin contains multiple skills — copy each into .agents/skills
      for (const skillName of readdirSync(skillsDir)) {
        const srcPath = join(skillsDir, skillName);
        const targetPath = join(agentsSkills, skillName);
        ...
        ui.success(`  [skills] ${skillName} installed from ${name}`);
      }
    } else if (existsSync(join(dest, 'SKILL.md'))) {
      // Plugin is a single skill — copy whole repo
      const targetPath = join(agentsSkills, name);
      if (existsSync(targetPath)) {
        rmSync(targetPath, { recursive: true, force: true });
      }
      cpSync(dest, targetPath, { recursive: true });
      ui.success(`  [skills] ${name} installed`);
    }
  } finally {
    rmSync(tmp, { recursive: true, force: true });
  }
...
export function installPlugin(name, source, selectedClis) {
  const clis = selectedClis || installedClis();
  // Always install for Claude first so the plugin registry is updated
  if (clis.includes('claude')) {
    installPluginForCli(name, source, 'claude');
  }
  // For every other CLI, extract plugin skills into the shared skill pool
  const others = clis.filter(c => c !== 'claude');
  if (others.length) {
    installPluginSkills(name, source);
  }
}
...
function bootstrapProject() {
  ...
  const entries = ['.env.local', '.venv', '.super/.super', '.kimi/.kimi', '.codex/.codex', '.claude/.claude', '.gemini/.gemini'];
