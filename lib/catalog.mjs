...
export function installedClis() {
  return ['claude', 'gemini', 'codex'].filter(isInstalled);
}
...
// Symlink super's built-in skills into Claude's GLOBAL skill dir
// (~/.claude/skills/). Points directly at $SUPER_HOME/skills/
// so every `git pull` in ~/.super (auto-update or manual) instantly refreshes
// the deployed skills — no `super install` re-run required. Skills invoked from
// outside a brain project (e.g. /super-clone from $HOME) stay current.
//
// Gemini and Codex are excluded: they read project-local .agents/skills directly.
// A ~/.<cli>/skills sync causes skill-name conflicts (every skill loads twice).
//
// Only overwrites entries that are ALREADY symlinks (refreshes stale targets).
// Real directories with matching names are left alone — those are the user's
// own custom skills that happen to collide.
export function ensureGlobalSuperSkills(selectedClis, opts = {}) {
  ...
  for (const cli of clis) {
    ...
    let linked = 0, refreshed = 0, kept = 0, collided = 0;
    for (const name of readdirSync(builtinDir)) {
      const src = join(builtinDir, name);
      try { if (!lstatSync(src).isDirectory()) continue; } catch { continue; }
      ...
      if (stat) {
        if (stat.isSymbolicLink()) {
          // Already a symlink. If it resolves to the same source, keep it.
          try {
            if (realpathSync(dest) === realpathSync(src)) { kept++; continue; }
          } catch { /* broken symlink — fall through to recreate */ }
          unlinkSync(dest);
          wasRefresh = true;
        } else {
          // Real dir with same name — user's custom skill, don't clobber
          collided++;
          continue;
        }
      }
      ...
      try {
        ...
        if (wasRefresh) refreshed++; else linked++;
      } catch (err) {
        ui.warn(`  [${cli}] failed to symlink ${name}: ${err.message}`);
      }
    }

    if (silent) continue;
    const total = linked + refreshed;
    if (total) {
      const parts = [];
      if (linked) parts.push(`${linked} new`);
      if (refreshed) parts.push(`${refreshed} refreshed`);
      ui.success(`  [${cli}] ~/.${cli}/skills: ${total} super skill${total === 1 ? '' : 's'} (${parts.join(', ')})`);
    } else if (kept) {
      ui.muted(`  [${cli}] ~/.${cli}/skills: ${kept} super skill${kept === 1 ? '' : 's'} already up-to-date`);
    }
    if (collided) {
      ui.muted(`  [${cli}] ~/.${cli}/skills: ${collided} name${collided === 1 ? '' : 's'} collided with a real dir — skipped`);
    }
  }
}
