...
function configureClaudeMarketplace(name, source, repoDir) {
  ...
  // Discover plugin names from marketplace.json
  const mktFile = join(repoDir, '.claude-plugin', 'marketplace.json');
  let pluginNames = [];
...
export function installPlugin(name, source, selectedClis) {
  ...
  // Clone the plugin repo once
  const repoDir = clonePluginRepo(source);
  if (!repoDir) { ui.warn(`  Failed to clone ${source}`); return; }
