...
// Symlink super's built-in skills into each installed CLI's GLOBAL skill dir
// (~/.claude/skills/, ~/.codex/skills/). Points directly at $SUPER_HOME/skills/
...
//
// Gemini is excluded: it reads project-local .agents/skills directly. A
// ~/.gemini/skills symlink causes skill-name conflicts (every skill loads twice).
//
...
export function ensureGlobalSuperSkills(selectedClis, opts = {}) {
  ...
  const clis = (selectedClis || installedClis()).filter(cli => cli !== 'gemini');
