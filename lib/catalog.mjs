...
export function installBuiltinSkills(selectedClis) {
  ...
  const clis = selectedClis || installedClis();
  const skills = readdirSync(builtinDir).filter(name => {
    const src = join(builtinDir, name);
    try { return lstatSync(src).isDirectory(); } catch { return false; }
  });

  // Per-CLI summary instead of one line per (skill × CLI). Keeps the install
  // log readable — 50+ "[claude] skill-x installed" lines was noise.
  for (const cli of clis) {
    const dir = skillDir(cli);
    mkdirSync(dir, { recursive: true });
    let copied = 0;
    ...
    for (const name of skills) {
      ...
      try {
        ...
        failures.push({ name, error: e?.message || 'unknown' });
      }
    }
    ui.success(`  [${cli}] ${copied} built-in skills installed`);
    for (const f of failures) ui.warn(`    failed: ${f.name} — ${f.error}`);
  }
}
...
// Copy `.env.local` into the project root if missing. First tries the
// project-local `.env.example`; falls back to the sample shipped with super
// at `<SUPER_HOME>/references/env.example`. Never overwrites. Warns if
// required keys are still blank.
export function scaffoldEnvLocal() {
  const root = findRoot();
  const envLocal = join(root, '.env.local');
  const sources = [
    join(root, '.env.example'),
    process.env.SUPER_HOME ? join(process.env.SUPER_HOME, 'references', 'env.example') : null,
  ].filter(Boolean);

  if (!existsSync(envLocal)) {
    const src = sources.find(p => existsSync(p));
    if (src) {
      ui.info(`  Creating .env.local from ${src}...`);
      writeFileSync(envLocal, readFileSync(src, 'utf8'));
