...
const OBSOLETE_CONFIG_SOURCES = [
  {
    from: 'marketingskills/marketing-skills',
    to: 'coreyhaines31/marketingskills',
    reason: 'marketing-skills moved to coreyhaines31/marketingskills',
  },
];

export function migrateObsoleteConfigSources() {
  const configPath = findConfig();
  if (!configPath || !existsSync(configPath)) return false;

  let raw;
  try { raw = readFileSync(configPath, 'utf8'); } catch { return false; }

  let updated = raw;
  const applied = [];
  for (const rule of OBSOLETE_CONFIG_SOURCES) {
    if (!updated.includes(rule.from)) continue;
  ...
  for (const rule of applied) {
    ui.success(`Updated ${configPath}: ${rule.from} → ${rule.to}`);
    ui.muted(`  ${rule.reason}`);
  }
  return true;
}
...
function configureClaudeMarketplace(name, source, repoDir) {
  ...
  if (pluginNames.length) {
    ...
    settings.enabledPlugins[`${pluginName}@${name}`] = true;
  }
