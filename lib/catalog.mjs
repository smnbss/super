...
import { findRoot, findConfig, catalogSystem, catalogClis, catalogSkills, catalogPlugins, catalogMcps, catalogCommands, mcpItem, loadDotEnv } from './config.mjs';
...
export function installSkill(name, source) {
...
export function installPlugin(name, source) {
  for (const cli of installedClis()) {
...
// ─── Command install ────────────────────────────────────────────────────────
// Commands are markdown files copied from a repo subpath to each CLI's
// native commands directory.

function commandDir(cli) {
  const root = findRoot();
  switch (cli) {
    case 'claude': return join(root, '.agents', 'commands');
    case 'gemini': return join(process.env.HOME, '.gemini', 'commands');
    case 'codex':  return join(process.env.HOME, '.codex', 'commands');
    case 'kimi':   return join(process.env.HOME, '.kimi', 'commands');
  }
}

function copyCommandsFromSource(name, source, destDir) {
  const parts = source.split('/');
  const [owner, repo] = parts;
  const subpath = parts.slice(2).join('/');

  if (!subpath) {
    ui.warn(`    ${name}: command source must include a subpath (owner/repo/path)`);
    return false;
  }
  ...
  try {
    ...
    const files = readdirSync(srcPath).filter(f => f.endsWith('.md'));
    if (files.length === 0) {
      ui.warn(`    No .md files found in ${subpath}`);
      rmSync(tmp, { recursive: true, force: true });
      return false;
    }

    for (const file of files) {
      cpSync(join(srcPath, file), join(destDir, file), { force: true });
    }
    rmSync(tmp, { recursive: true, force: true });
    return true;
  } catch {
    ui.warn(`    Failed to sync commands for ${name}`);
    return false;
  }
}

function installCommandForCli(name, source, cli) {
  const dir = commandDir(cli);
  ui.info(`  [${cli}]`);
  if (copyCommandsFromSource(name, source, dir)) {
    ui.success(`    ${name} commands synced`);
  }
}

export function installCommand(name, source) {
  for (const cli of installedClis()) {
    installCommandForCli(name, source, cli);
  }
}
...
export function installMcp(name, envMap) {
...
export function installEnabled() {
  ...
  for (const skill of catalogSkills()) {
    ...
    installSkill(skill.name, skill.source);
  }
  ...
  for (const plugin of catalogPlugins()) {
    ...
    installPlugin(plugin.name, plugin.source);
  }

  ui.spacer();
  ui.brand('Installing enabled commands...');
  ui.spacer();
  for (const cmd of catalogCommands()) {
    if (!cmd.enabled) continue;
    installCommand(cmd.name, cmd.source);
  }
