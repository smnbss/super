...
function ensureClaudeSkillsDirectory() {
  ...
  ui.success(`  .claude/skills populated (${linked} skills, ${_harvestedSkills.size} harvested hidden from Claude)`);
}
...
function ensureClaudeCommandsDirectory() {
  ...
  if (linked) ui.success(`  .claude/commands populated (${linked} commands)`);
}
...
function clonePluginRepo(source) {
  ...
  try {
    ...
    ui.warn(`  Failed to clone ${owner}/${repo}`);
    rmSync(tmp, { recursive: true, force: true });
    return null;
  }
}
