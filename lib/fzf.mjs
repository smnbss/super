...
import { sessionsDir, activePtr } from './config.mjs';
...
async function fallbackPickSession(from) {
  ...
  const options = sessions.map(s => {
    ...
    return `${s.title.padEnd(38)}  ${s.started.padEnd(20)}  ${s.turns} turns${marker}`;
  });
...
export async function fzfPickCli() {
  const clis = ['claude', 'gemini', 'codex', 'kimi'].filter(isInstalled);
