...
function fzfPickSessionFzf(from) {
  ...
  const lines = sessions.map(s => {
    ...
    return `${s.filename.replace('.md', '')}|${cliIcon}|${s.title}|${age}|${s.turns} turns${modelInfo}${marker}`;
  }).join('\n');

  try {
    const selected = execSync(
      `echo "${lines}" | fzf --header="Select session (ctrl-n for new)" --delimiter='|' --with-nth=2,3,4,5 --height=60% --border=rounded --prompt="Session > " --bind='ctrl-n:abort'`,
...
async function fallbackPickSession(from) {
  ...
  const options = sessions.map(s => {
    ...
    const cliIcon = { claude: '🟠', gemini: '🔵', codex: '🟢', antigravity: '🪽' }[s.cli] || '⚪';
    const modelInfo = s.model ? ` [${s.model}]` : '';
    return `${s.started.padEnd(20)}  ${cliIcon} ${String(s.turns).padStart(2)} turns  ${s.title.padEnd(40)}${modelInfo}${marker}`;
  });
...
export async function fzfPickCli() {
  const clis = ['claude', 'gemini', 'codex', 'antigravity'].filter(isInstalled);
