...
⚠️ **ORCHESTRATOR ONLY → `references/work-list.md`.** Read it before dispatching anything.

**The one-line rule a WORKER needs:** you were handed a target doc and a delta base. That base is
the doc's own recorded `head:`, chosen because the doc is behind its clone — **not** because the
repo happened to move today. **An empty divergence set is a complete answer; an empty movement
ledger is not.**
