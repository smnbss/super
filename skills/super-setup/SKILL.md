...
The current directory (or an ancestor) must contain a **real** `.super/` directory — that marks the project root and **is** the brain. If no real `.super/` is found walking up from cwd, tell the user to run `super install` first and stop.
...
Walk up from cwd until you hit a `.super/` directory **that is a real directory** (check with `ls -ld <candidate>/.super` — a leading `d` means real, `l` means symlink). Skip any `.super` that is a symlink (that's a debug link). The same rule applies to `.claude/.claude`, `.codex/.codex`, `.gemini/.gemini` — never step through those. The first real `.super/` match is the brain root. Show the user: *"Brain root: <path>. Continue here? Yes / No (cancel)."*
